				 ...
				 bool is_pcie, bool is_cxl)
{
	...
	/*
	 * If a CXL host bridge device exposes CXL_OSC, CXL aware OSPM shall
	 * evaluate CXL_OSC and not evaluate PCIe_OSC. (CXL 2.0 sec 9.15.1.1.2)
	 */
	if (is_cxl) {
		...
		if (pci_cxl_port_reg_enabled())
			cxl_support |= OSC_CXL_PORT_REG_ACCESS_SUPPORT;
